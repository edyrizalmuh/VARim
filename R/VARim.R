#' VAR-IM
#'
#' This function performs an imputation using Vector Autoregression Imputation
#' Method (VAR-IM), a model-based imputation method for multivariate time series data.
#'
...
#' imp_data_OneDummy = VARim(miss_data, dummy = dummy_mat)
#'
#' # For dummy with multiple variables:
#' dummy_mats = cbind(rnorm(num_row,0,1), rnorm(num_row,1,4))
#' imp_data_MultipleDummy = VARim(miss_data, dummy = dummy_mats)
#'
VARim <- function(miss_data, ic = "AIC", min_norm = 0.05, max_iter = 10,
